...
class apb_virtual_32b_write_read_seq extends apb_virtual_base_seq;
  ...
  apb_master_32b_read_seq apb_master_32b_read_seq_h[NO_OF_MASTERS];

  //Variable: apb_slave_32b_write_seq_h
  //Instatiation of apb_slave_32b_write_seq
  apb_slave_32b_write_seq apb_slave_32b_write_seq_h[TOTAL_SLAVES];
...
task apb_virtual_32b_write_read_seq::body();
  super.body();
	...
	foreach(apb_master_32b_write_seq_h[i]) begin
	  apb_master_32b_write_seq_h[i] = apb_master_32b_write_seq::type_id::create($sformatf("apb_master_32b_write_seq_h[%0d]", i));
	end

	foreach(apb_master_32b_read_seq_h[i]) begin
  	apb_master_32b_read_seq_h[i] = apb_master_32b_read_seq::type_id::create($sformatf("apb_master_32b_read_seq_h[%0d]", i));
	end

	foreach(apb_slave_32b_write_seq_h[i]) begin
  	apb_slave_32b_write_seq_h[i]=apb_slave_32b_write_seq::type_id::create($sformatf("apb_slave_32b_write_seq_h[%0d]", i));
		if(!apb_slave_32b_write_seq_h[i].randomize() with {choose_packet_data_seq == 0;}) begin
    	`uvm_error(get_type_name(), "Randomization failed : Inside apb_virtual_32b_write_read_seq")
		end
	end

	foreach(apb_slave_32b_read_seq_h[i]) begin
  	apb_slave_32b_read_seq_h[i]=apb_slave_32b_read_seq::type_id::create($sformatf("apb_slave_32b_read_seq_h[%0d]", i));
    if(!apb_slave_32b_read_seq_h[0].randomize() with {choose_packet_data_seq == 0;}) begin
    	`uvm_error(get_type_name(), "Randomization failed : Inside apb_virtual_32b_write_read_seq")
    end
	end
  ...
  fork
    begin
      foreach(apb_master_32b_read_seq_h[i]) begin
				if(!apb_master_32b_read_seq_h[i].randomize() with { address_seq inside {[slave_addr.min_addr[i]:slave_addr.max_addr[i]]};}) begin
					`uvm_error(get_type_name(), $sformatf("Randomization failed for master %0d", i))
				end
			end
