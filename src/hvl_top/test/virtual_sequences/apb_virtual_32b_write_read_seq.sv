...
class apb_virtual_32b_write_read_seq extends apb_virtual_base_seq;
  ...
  apb_master_sequence apb_master_32b_write_seq_h[NO_OF_MASTERS], apb_master_32b_read_seq_h[NO_OF_MASTERS];

  //Variable: apb_slave_32b_write_seq_h
  //Instatiation of apb_slave_32b_write_seq
  apb_slave_sequence apb_slave_32b_write_seq_h[TOTAL_SLAVES], apb_slave_32b_read_seq_h[TOTAL_SLAVES];
...
task apb_virtual_32b_write_read_seq::body();
  super.body();
	...
	foreach(apb_master_32b_read_seq_h[i]) begin
  	apb_master_32b_read_seq_h[i] = apb_master_sequence::type_id::create($sformatf("apb_master_32b_read_seq_h[%0d]", i));
	end
	...
	// Write sequnce
  foreach(apb_master_32b_write_seq_h[i]) 
		if(!apb_master_32b_write_seq_h[i].randomize() with { 
  ...
  fork
    ...
    begin
      foreach(apb_master_32b_write_seq_h[i]) begin
        fork
          automatic int j =i;
          apb_master_32b_write_seq_h[j].start(p_sequencer.apb_master_seqr_h[j]);
        join_none
      end
    end
 	  wait fork;
