...
task apb_virtual_24b_write_read_seq::body();
  ...
  fork
    begin
      ...
      fork
        foreach(apb_master_24b_write_seq_h[i]) begin
          automatic int idx = i;
          apb_master_24b_write_seq_h[idx].start(p_sequencer.apb_master_seqr_h[idx]);
        end
      join_none
  ...
  fork
    begin
      foreach(apb_master_24b_read_seq_h[i]) begin
				if(!apb_master_24b_read_seq_h[i].randomize() with { address_seq inside {[slave_addr.min_addr[i]:slave_addr.max_addr[i]]};}) begin
					`uvm_error(get_type_name(), $sformatf("Randomization failed for master %0d", i))
				end
			end
